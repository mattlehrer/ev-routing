	...
	while (lTemp.size() > 0) {
		...
		g.forEachLinkedNode(
			...
			(node, link) => {
				...
				let edgePower = 0;
				// and add financial cost
				let edgeFinancialCost = 0;

				// calculations are based on the node type
				if (['s', 'a', 'b', 'i'].includes(node.data.type)) {
					...
					edgePower = -1 * (node.data.chargeLevel * batteryCapacity - soc);

					// calculate the financial cost of charging
					edgeFinancialCost =
						(edgeDuration * (node.data.costMin ?? 0)) / 60 - edgePower * (node.data.costKwh ?? 0);
				} else if (node.data.type === 'd') {
					throw new Error('There should not be outgoing edges from the destination');
				}

				const newCumulativeDuration = lCurrent.cumulativeDuration + edgeDuration;

				const newCumulativePower = Math.min(batteryCapacity, lCurrent.cumulativePower + edgePower);
